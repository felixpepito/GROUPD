...
@section('content')
   ...
   function addToOrder(name, price, imageUrl) {
    ...
    if (existingRow) {
        ...
        deleteButton.classList.add('delete-button'); // Add a CSS class to the button
        deleteButton.onclick = function() {
            var rowToDelete = this.parentNode.parentNode;
            var itemPrice = parseFloat(rowToDelete.cells[1].textContent.replace('₱', ''));
            orderList.removeChild(rowToDelete); // Remove the row from the table
            updateTotalPrice(-itemPrice); // Subtract the item price from the total
        };
